...
class TestSaveAndCreateAbTestButton(WagtailTestUtils, TestCase):
    ...
    def test_click_on_page_edit(self):
        ...
        self.assertRedirects(
            response,
            reverse(
                "wagtail_ab_testing_admin:add_ab_test_compare", args=[self.page.id]
            ),
        )
    ...
    def test_doesnt_show_on_page_create(self):
        response = self.client.get(
            reverse(
                ...
                args=["wagtail_ab_testing_test", "simplepage", self.page.id],
            )
        )
...
class TestAddAbTestCompareView(WagtailTestUtils, TestCase, PermissionTests):
    ...
    def get(self, page_id):
        return self.client.get(
            reverse("wagtail_ab_testing_admin:add_ab_test_compare", args=[page_id])
        )
...
class TestAddAbTestFormView(WagtailTestUtils, TestCase, PermissionTests):
    ...
    def get(self, page_id):
        return self.client.get(
            reverse("wagtail_ab_testing_admin:add_ab_test_form", args=[page_id])
        )
    ...
    def test_get_add_form(self):
        ...
        self.assertEqual(
            json.loads(response.context["goal_selector_props"]),
            {
                ...
                "globalGoalTypes": [{"name": "Global Event", "slug": "global-event"}],
            },
        )
    ...
    def test_post_add_form(self):
        ...
        self.assertEqual(
            ab_test.hypothesis,
            'Does changing the title to "Donate now!" increase donations?',
        )
